...
// Description	: The file contains Class definition for TString.
...
public:
        TString(const char *pText = 0); // default ctor . // We need to include the default values here and not at cpp file.
        //TString(const char *pText); // default ctor
	TString(const TString& SObject) ; //Copy Constructor
        ~TString(); //Destructor

        int length() const; //since length() is not changing class object, it is defined as const and same for others.

        char *asChar() const;

        void assign(const TString& SObject); // since assign() is changing class object, it is not defined as const.
